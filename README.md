...
- **只保留本周余量**：Codex 已取消 5 小时限制，界面不再显示 5 小时额度。
- **更醒目的余量卡片**：主视觉只显示本周剩余百分比，并配一条进度条。
- **显示更新细节**：右下角显示“刚刚更新 / x分钟前更新 / x小时前更新”，来自 Codex 日志中的真实 `timestamp`。
- **截止时间只显示日期**：周额度显示为“截止到 x月x日”，来源于 Codex 日志中的 `resets_at`。
- **ChatGPT 在线状态**：右上角红绿点检测本机 ChatGPT 桌面程序是否正在运行。
- **自动重新同步**：如果 ChatGPT 从关闭变为开启，CodexQuota 会自动重新读取本地日志并同步本周余量。
- **不伪造实时数据**：没有新的 Codex `token_count` 日志时，程序不会自己编造额度变化。
